...
def get_train_batches(x, y=None, r=None, batch_size=-1, shuffle=False):
    n = x.shape[0]
    if batch_size < 0:
        batch_size = n
    indxs = np.arange(n)
    ...
    for i in range(0, n, batch_size):
        ...
        yield x[indxs[i:et], :], None if y is None else y[indxs[i:et]], None if r is None else r[indxs[i:et]]
...
class GAN(object):
    ...
    def __init__(self, data_dim=1, discr_layer_nodes=None, discr_layer_activations=None,
                 ...
                 iso_gan=False, iso_gan_lambda=1.0, iso_model=None, iso_gan_use_gen=False,
                 info_gan=False, info_gan_lambda=1.0,
        ...
        :param iso_gan: bool
            if True, then use IsoGAN, else simple or conditional GAN
            'IsoGAN' name comes from Isolation Forest GAN. We try to predict the
            Isolation Forest scores with the hope that the discriminator features
            will learn a better latent representation that avoids mode-collapse.
            This feature is purely EXPERIMENTAL.
        :param iso_gan_lambda: float
            IsoGAN regularization penalty
        :param iso_gan_use_gen: bool
            if False and iso_gan, then add IsoGAN loss only to discriminator output
            if True and iso_gan, then add IsoGAN loss to generator output also
        :param info_gan: bool
        ...
        self.iso_gan = iso_gan
        self.iso_gan_lambda = iso_gan_lambda
        self.iso_model = iso_model
        self.iso_gan_use_gen = iso_gan_use_gen
        ...
        # Iso GAN (IsoGAN)
        self.r = None
        self.r_data_network = self.r_gen_network = self.r_pred = None
        self.iso_gen_loss = tf.constant(0.0)
        self.iso_data_loss = tf.constant(0.0)
    ...
    def init_network(self):
        ...
        discr_data_logit = get_nn_layer(self.discr_data, layer_from_top=1)
        if not self.label_smoothing:
            discr_loss_data = -tf.log(tf.nn.sigmoid(discr_data_logit))
        else:
            logger.debug("Label smoothing enabled with smoothing probability: %f" % self.smoothing_prob)
            discr_loss_data = tf.nn.sigmoid_cross_entropy_with_logits(logits=discr_data_logit,
                                                                      labels=tf.ones(shape=tf.shape(discr_data_logit)) * self.smoothing_prob)
        ...
        if self.iso_gan:
            # prepare self.r, self.r_gen_network, self.r_data_network,
            # self.iso_data_loss, self.iso_gen_loss
            self.init_iso_gan()

        if self.info_gan:
            # prepare self.q_network, self.info_gan_loss
            self.init_info_gan()
        ...
        r_params = [v for v in vars if v.name.startswith('IsoGAN/')]
        q_params = [v for v in vars if v.name.startswith('InfoGAN/')]

        if self.iso_gan and len(r_params) == 0:
            # Just to be sure we do not have programmatic errors
            raise ValueError("No r_params found for IsoGAN")
        ...
        if self.l2_lambda > 0:
            ...
            l2_loss_g, l2_loss_d, l2_loss_r, l2_loss_q = self.get_l2_regularizers(g_params, d_params,
                                                                                  r_params=r_params,
                                                                                  q_params=q_params)
            ...
            if self.iso_gan:
                if self.iso_gan_use_gen:
                    self.iso_gen_loss += self.l2_lambda * l2_loss_r
                    g_params.extend(r_params)
                self.iso_data_loss += self.l2_lambda * l2_loss_r
                d_params.extend(r_params)
        ...
        self.gen_training_op = self.training_op(self.gen_loss +
                                                self.iso_gan_lambda * self.iso_gen_loss +
                                                self.info_gan_lambda * self.info_gan_loss,
                                                var_list=g_params, use_adam=self.use_adam)
        self.discr_training_op = self.training_op(self.discr_loss +
                                                  self.iso_gan_lambda * self.iso_data_loss +
                                                  self.info_gan_lambda * self.info_gan_loss,
    ...
    def get_l2_regularizers(self, g_params, d_params, r_params=None, q_params=None):
        ...
        :param r_params: list of tf.Variable
            The IsoGAN regularization parameters
        ...
        """
        l2_loss_g = l2_loss_d = l2_loss_r = l2_loss_q = 0.0
        for v in g_params:
        ...
        if r_params is not None:
            for v in r_params:
                l2_loss_r += tf.nn.l2_loss(v)
        if q_params is not None:
            for v in q_params:
                l2_loss_q += tf.nn.l2_loss(v)
        return l2_loss_g, l2_loss_d, l2_loss_r, l2_loss_q
    ...
    def init_iso_gan_network(self, x, reuse=False):
        return self.gan_construct(x, n_neurons=[1], names=["r_out"],
                                  activations=[None], reuse=reuse)

    def init_iso_gan(self):
        logger.debug("Adding IsoGAN regularization")
        self.r = tf.placeholder(tf.float32, shape=(None, 1), name="r")
        discr_gen_feats = get_nn_layer(self.discr_gen, layer_from_top=2)
        discr_data_feats = get_nn_layer(self.discr_data, layer_from_top=2)
        with tf.variable_scope("IsoGAN"):
            self.r_gen_network = self.init_iso_gan_network(discr_gen_feats, reuse=False)
            self.r_data_network = self.init_iso_gan_network(discr_data_feats, reuse=True)
        r_gen_out = get_nn_layer(self.r_gen_network, layer_from_top=1)
        r_data_out = get_nn_layer(self.r_data_network, layer_from_top=1)
        if self.iso_gan_use_gen:
            self.iso_gen_loss = tf.nn.l2_loss(tf.subtract(self.r, r_gen_out))
        self.iso_data_loss = tf.nn.l2_loss(tf.subtract(self.r, r_data_out))
    ...
    def init_ano_gan_network(self, x=None, y=None, z=None):
        ...
        # Might be redundant if reuse=True already before entering this method.
        ano_gan_net_G = self.generator(z=z, y=y, reuse_gen=True)
        ano_gan_G = get_nn_layer(ano_gan_net_G, layer_from_top=1)
    ...
    def ano_gan_outputs(self):
        ...
        """
        return get_nn_layer(self.ano_gan_net_G, layer_from_top=1), \
               get_nn_layer(self.ano_gan_net_D, layer_from_top=1), \
               get_nn_layer(self.ano_gan_net_D, layer_from_top=2) if self.info_gan else None

    def init_ano_gan(self):
    ...
    def fit(self, x, y=None, r=None):
        ...
        :param y: np.ndarray(shape=(n_instances,n_classes))
            Used only by ConditionalGAN
        :param r: np.ndarray(shape=(n_instances,n_response_cols))
            Used only by IsoGAN
        :return:
        """
        ...
        if self.iso_gan and (r is None):
            raise ValueError("IsoGAN fit() requires response r")

        fit_tm = Timer()
        for epoch in range(self.n_epochs):
            tm = Timer()
            i = 0
            for x_batch, y_batch, r_batch in get_train_batches(x, y=y, r=r, batch_size=self.batch_size, shuffle=self.shuffle):
                ...
                if self.iso_gan:
                    feed_dict_discr.update({self.r: r_batch})

                if self.conditional:
                    feed_dict_discr.update({self.y: y_batch})

                self.session.run([self.discr_training_op], feed_dict=feed_dict_discr)
                # logger.debug("iso_loss: %f" % iso_loss)
                ...
                if i % 1 == 0:  # train gen_loss only half as frequently as discr_loss
                    ...
                    if self.conditional:
                        feed_dict_gen.update({self.y: y_batch})
                    if self.iso_gan and self.iso_gan_use_gen:
                        gen_out = get_nn_layer(self.gen, layer_from_top=1)
                        x_gen = self.session.run([gen_out], feed_dict=feed_dict_gen)[0]
                        r_gen_batch = np.reshape(self.iso_model.decision_function(x_gen), (-1, 1))
                        feed_dict_gen.update({self.r: r_gen_batch})
...
def get_gan_option_list():
    ...
    parser.add_argument("--iso_gan", action="store_true", default=False,
                        help="Whether to use IsoGAN")
    parser.add_argument("--iso_gan_lambda", action="store", type=float, default=1.0,
                        help="The IsoGAN penalty term")
...
class GanOpts(object):
    def __init__(self, args):
        ...
        self.iso_gan = args.iso_gan
        self.iso_gan_lambda = args.iso_gan_lambda
    ...
    def get_opts_name_prefix(self):
        ...
        iso_gan_sig = "_iso" if self.iso_gan else ""
        ...
        algo_sig = "%s%s%s_gan" % (cond_sig, iso_gan_sig, info_gan_sig)
