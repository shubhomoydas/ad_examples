...
def aad_batch():
    ...
    if run_aad:
        ...
        for runidx in opts.get_runidxs():
            ...
            baseline_w = mdl.get_uniform_weights()

            agg_scores = mdl.get_score(X_train_new, baseline_w)
            ...
            # mdl.init_weights(init_type=opts.init, samples=X_train_new)
            mdl.init_weights(init_type=opts.init, samples=None)
            ...
            if metrics is not None:
                ...
                if False:
                    debug_qvals(X_train_new, mdl, metrics, args.resultsdir, opts)
            else:
